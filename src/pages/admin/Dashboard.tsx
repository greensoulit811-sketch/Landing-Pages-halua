...
import { useCheckoutLeads } from '@/hooks/useCheckoutLeads';
...
const Dashboard = () => {
  ...
  const { data: checkoutLeads = [] } = useCheckoutLeads();

  const totalRevenue = checkoutLeads.filter(l => l.status === 'completed').reduce((s, l) => s + Number(l.cart_total || 0), 0);

  const pendingOrders = checkoutLeads.filter(l => l.status === 'pending_checkout').length;
  const deliveredOrders = checkoutLeads.filter(l => l.status === 'completed').length;
  ...
  const statusData = [
    { name: 'Pending', value: checkoutLeads.filter(l => l.status === 'pending_checkout').length },
    { name: 'Abandoned', value: checkoutLeads.filter(l => l.status === 'abandoned').length },
    { name: 'Completed', value: checkoutLeads.filter(l => l.status === 'completed').length },
  ].filter(d => d.value > 0);
  ...
  const combinedRecent = checkoutLeads.map(l => ({
    id: l.id,
    number: 'ORD-' + l.id.substring(0, 4).toUpperCase(),
    customer: l.customer_name,
    total: Number(l.cart_total || 0),
    status: l.status,

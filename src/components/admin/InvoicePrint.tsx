...
export const printInvoice = (order: any) => {
  const items = (order.items || order.cart_items || []) as any[];
  const orderNumber = order.order_number || order.id?.substring(0, 8).toUpperCase() || 'ORDER';
  const totalAmount = order.total || order.cart_total || 0;
  const html = `
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <div class="info-grid">
    ...
    <div class="info-block">
      ...
      <p><strong>Payment:</strong> ${order.payment_method ? order.payment_method.toUpperCase() : 'CASH ON DELIVERY'}</p>
...
export const printCourierSlip = (order: any) => {
  const items = (order.items || order.cart_items || []) as any[];
  const orderNumber = order.order_number || order.id?.substring(0, 8).toUpperCase() || 'ORDER';
  const totalAmount = order.total || order.cart_total || 0;
  const html = `
...
<html>
<head>
  <title>Courier Slip - ${orderNumber}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
